...
**Decision update:** Vercel was selected and deployed on 2026-08-20. The host comparison below is retained as decision history; Cloudflare Pages is no longer the selected V1 host.
...
Use **Vercel deployed from GitHub** for V1, and use **OpenFreeMap's `liberty`
MapLibre style** for the initial basemap. The live deployment proved the Vite
build and static data-generation path, so changing hosts would add work without
improving V1.
...
3. Do not add geocoding, routing, satellite imagery, or static-map APIs in V1.
   ...
   cost/terms.
4. Track two signals after launch: Vercel deployment/usage status and
   tile-provider availability/errors. There is no evidence here of expected
   CherryMap traffic, so free-tier capacity and ongoing zero cost are not
   guaranteed.
